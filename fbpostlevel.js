...
const getFacePostData = async (idpage, token) => {
		...
		//url fetch and data transform
				...
				class newPost {
					constructor(id, idpage, created_time, post_impressions_paid, post_impressions_paid_unique, post_impressions_organic, post_impressions_organic_unique, likes, shares, comments, others_clicks, photo_clicks, link_clicks, caption, url) {
						this.id = id
						this.idpage = idpage
				...
				dados.forEach(el => {
					...
					dadosFinal.push(new newPost(postId,idpage, createdTime, paidPostImpressions, paidPostImpressionsUnique, organicPostImpressions, organicPostImpressionsUnique, likes, shares, comments, otherClicks, photoClicks, linkClicks, caption, url))
				})
		...
		//========================== Step 2: Recording data on DB ===========================
			...
			if (items.length == 0) {
				...
				console.log(`O(s) novo(s) registros, no valor total de ${newData.length}, foram carregados na db com sucesso`);
			} else {
				console.log(
			...
			if (newData.length == 0) {
				...
				setTimeout(() => console.log(`Os registros existentes correspondentes com a nova lista serão atualizados`), 3000);
				...
				dadosFinal.some((item2) => {
					if (item1.id == item2.id) {
						...
						item1.post_impressions_paid_unique = item2.post_impressions_paid_unique; // update post_impressions_paid_unique value on db
						item1.post_impressions_organic = item2.post_impressions_organic; // update post_impressions_organic value on db
						item1.post_impressions_organic_unique = item2.post_impressions_organic_unique; // update post_impressions_organic_unique value on db
						...
						setTimeout(() => console.log(`Registro de id ${item1.id} atualizado com sucesso`), 4000);
						}
					}		
				))
