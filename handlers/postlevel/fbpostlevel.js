...
const getFacePostData = async (idpage, token) => {
  ...
  const url = `https://graph.facebook.com/v22.0/${idpage}?fields=published_posts.since(2024-02-01).until(2025-03-01){id,created_time,insights.metric(post_impressions_paid,post_impressions_paid_unique, post_impressions_organic,post_impressions_organic_unique,post_reactions_by_type_total, post_activity_by_action_type, post_clicks_by_type), message, permalink_url}&access_token=${token}`;
