...
const getInstaPageData = async (pageId, token) => {
  //========================== Step 1: Getting data via API ===========================
		...
		const url = `https://graph.facebook.com/v14.0/${pageId}/insights?metric=follower_count, profile_views, website_clicks&period=day&since=2023-04-01&until=2023-04-28&access_token=${token}`;
		...
		const newData = result.filter(
			(item) => !items.find((item2) => item.id == item2.id && item.page_id == item2.page_id)
		);
