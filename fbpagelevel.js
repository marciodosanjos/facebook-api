...
const getFacePageData = async (pageid, token) => {
		...
		const linkClicks = rawClicks.map(el => el.value["link clicks"] == undefined ? 0 : el.value["link clicks"]);

		//clicks_video_play
		const videoPlayClicks = rawClicks.map(el => el.value["video play"] == undefined ? 0 : el.value["video play"]);

		//other_clicks
		const otherClicks = rawClicks.map(el => el.value["other clicks"] == undefined ? 0 : el.value["other clicks"]);

		//new_page_fans_paid
		const rawPageFans = data[8].values;
		const pageFansPaid = rawPageFans.map(el => el.value["paid"] == undefined ? 0 : el.value["paid"]);

		//new_page_fans_organic
		const pageFansOrganic = rawPageFans.map(el => el.value["unpaid"] == undefined ? 0 : el.value["unpaid"]);
